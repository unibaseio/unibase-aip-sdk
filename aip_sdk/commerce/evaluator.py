...
class SchemaEvaluator:
    """
    Tier 2 Evaluator: Automatically verifies job deliverables against a JSON Schema.
    This mimics the standardized verification logic seen in platforms like Virtuals.
    """

    def __init__(self, job_market: JobClient, evaluator_id: str):
        self.market = job_market
    ...
    async def verify_and_settle(
        ...
        """
        Fetches the job, validates the deliverable data, 
        and triggers on-chain completion/rejection.
        """
        try:
            # 1. Fetch current job state
            job = await self.market.get(job_id)
            if job["status"] != "submitted":
                logger.warning(f"Job {job_id} is not in 'submitted' state. Current: {job['status']}")
                return False
            ...
            deliverable_data = job.get("metadata", {}).get("deliverable_data")
            
            if not deliverable_data:
                await self._reject(job_id, "Missing deliverable data in job metadata.")
                return False
            ...
            if is_valid:
                logger.info(f"Job {job_id} passed verification.")
                ...
                logger.warning(f"Job {job_id} failed verification.")
                return await self._reject(job_id, "Deliverable does not match the required schema.")
    ...
    async def _reject(self, job_id: str, reason: str) -> bool:
        return await self.market.aip.client.post(
            f"/v1/jobs/{job_id}/reject?rejector_id={self.evaluator_id}",
            json={"reason": reason}
        )
